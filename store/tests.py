...
class ProductCreateTestCase(APITestCase):
    def test_create_product(self):
        ...
        product_attrs = {'name': 'New Product',
...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        self.client.patch(f'/api/v1/products/{product.id}/', {
            'name': 'New Product',
            'description': 'Awesome product',
            'price': 123.45
        }, format='json')
    ...
    def test_upload_product_image(self):
        ...
        try:
            ...
            expected_photo_path = os.path.join(settings.MEDIA_ROOT, 'products',
                                               'vitamin-iron')
            self.assertTrue(updated.photo.path.startswith(expected_photo_path))
        finally:
